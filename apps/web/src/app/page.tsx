...
export default async function Home () {
  ...
  const { data: chats } = await supabase
    .from('chats')
    .select('user:user_id(id, vectorProvider:vector_db_selected), id, name, messages, folderId:folder_id, model, temperature, maxTokens:max_tokens, prompt, isAgent:is_agent, agent:agent_id(id, docsId:docs_id, prompt)')
